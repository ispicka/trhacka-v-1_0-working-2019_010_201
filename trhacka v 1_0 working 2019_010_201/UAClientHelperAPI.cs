...
#pragma warning restore CS0168 // The variable 'e' is declared but never used
        ...
        public MonitoredItem AddMonitoredItem(Subscription subscription, List<string> nodeIdStrings, List<string> itemNames, int samplingInterval)
        {
            //Create a monitored item
            string itemName = "";
            string nodeIdString = "";
        ...
        /// <exception cref="Exception">Throws and forwards any exception with short error description.</exception>
        /// 
        public MonitoredItem AddMonitoredItem(Subscription subscription, string nodeIdString, string itemName, int samplingInterval, MonitoredItemNotificationEventHandler eventHandler)
        {
            //Create a monitored item
            MonitoredItem monitoredItem = new MonitoredItem();
            //Set the name of the item for assigning items and values later on; make sure item names differ
            ...
            monitoredItem.DiscardOldest = true;
            //Define event handler for this item and then add to monitoredItem
            monitoredItem.Notification += new MonitoredItemNotificationEventHandler(eventHandler);
            try
            {
                //Add the item to the subscription
                subscription.AddItem(monitoredItem);
        ...
        private static ApplicationConfiguration CreateClientConfiguration()
        {
            ...
            configuration.ApplicationUri = "urn:Bintell"; //Kepp this syntax
            configuration.ProductUri = "Bintell";
            configuration.TraceConfiguration = new TraceConfiguration
            {
                OutputFilePath = "c:\\tracelog.txt",
                DeleteOnLoad = false,
        ...
        private static List<object> ParseTypeDictionary(String xmlStringToParse, String stringToParserFor)
        {
            ...
            //xmlStringToParse = xmlStringToParse.Remove(xmlStringToParse.Length);
